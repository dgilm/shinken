            ...
            for cfg_id in r.confs:
                ...
                try:
                    for kind in ('reactionner', 'poller', 'broker', 'receiver'):
                        ...
                        n_managed = len(r.to_satellites_managed_by[kind][cfg_id])
                        n_realm = r.get_nb_of_must_have_satellites(kind)
                        if n_managed < n_realm:
                            logger.warning("Missing satellite %s for configuration %d: "
                                           "n_managed=%s n_realm=%s" % (
                                kind, cfg_id, n_managed, n_realm))
